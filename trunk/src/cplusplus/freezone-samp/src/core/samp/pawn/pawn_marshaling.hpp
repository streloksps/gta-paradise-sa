...
namespace pawn {
    ...
    protected:
        ...
        marh_base(std::string const& name, marh_collection_t& marh_collection): marh_amx_t(name, base::get_pawn_call_args(), marh_collection) {
            cells[0] = params_count * sizeof(cell);
        }
        marh_base(std::string const& name): marh_amx_t(name, base::get_pawn_call_args()) {
            cells[0] = params_count * sizeof(cell);
        }
