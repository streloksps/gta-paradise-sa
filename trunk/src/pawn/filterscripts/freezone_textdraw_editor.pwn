...
public OnFilterScriptInit() {
    ...
    print("TextDraw editor for freemode Loaded");
    return 1;
}
...
save_textdraws_to_file(File: handle) {
    new tmp[1024];
    new aligment_arr[][] = {"left", "justified", "center", "right"};
    new fontstyle_arr[][] = {"style0", "style1", "style2", "style3"};
    new bool_arr[][] = {"false", "true"};
    for(new i;i<95;i++) {
        if (!textdrawactive[i]) {
			continue;
        }
        //textcolor[i] = 0x12345678;
        format(tmp,	sizeof(tmp), "unnamed%02d_textdraw=<x %.1f/><y %.1f/><letter_size_x %.2f/><letter_size_y %.2f/><color 0x%02x%06x/><alignment %s/><background_color 0x%02x%06x/><font_style %s/><is_proportional %s/><shadow %d/><outline %d/><box_is_use %s/><box_color 0x%02x%06x/><box_x %.2f/><box_y %.2f/>%s\r\n"
		,i
		,textx[i]
		,texty[i]
		,lettersizex[i]
		,lettersizey[i]
		,((textcolor[i] & 0xff000000) >> 24) & 0xff
		,textcolor[i] & 0xffffff
		,aligment_arr[alignment[i]]
		,((backcolor[i] & 0xff000000) >> 24) & 0xff
		,backcolor[i] & 0xffffff
		,fontstyle_arr[font[i]]
		,bool_arr[proportional[i]?1:0]
		,shadow[i]
		,outline[i]
		,bool_arr[box[i]?1:0]
		,((boxcolor[i] & 0xff000000) >> 24) & 0xff
		,boxcolor[i] & 0xffffff
		,boxsizex[i]
		,boxsizey[i]
		,textdrawtext[i]
...
public OnPlayerText(playerid, text[])
{
    ...
    if(saving)
        ...
        new str[256], File:gFile;
        format(str,256,"%s.txt",text);
        gFile = fopen(str, io_write);
        save_textdraws_to_file(gFile);
        fclose(gFile);
        TogglePlayerControllable(playerid,1);
        new str2[256];
...
{
    SetTimerEx("HoldKey", 300, 0, "d", playerid);
    ...
    if(lettersizing && newkeys == KEY_WALK && textx[currtextdraw] <= 640.0)
        {
        lettersizex[currtextdraw]+=0.025;
        UpdateTextDraw(currtextdraw,0);
        }
    ...
    if(lettersizing && newkeys == KEY_JUMP && texty[currtextdraw] >= 0.0)
        {
        lettersizey[currtextdraw]-=0.025;
        UpdateTextDraw(currtextdraw,0);
        }
...
public HoldKey(playerid)
{
    ...
    if(movingtext && keys == KEY_WALK && textx[currtextdraw] <= 640.0)
    {
        textx[currtextdraw]+=1.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_LOOK_BEHIND && textx[currtextdraw] >= 0.0)
    {
        textx[currtextdraw]-=1.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_JUMP && texty[currtextdraw] >= 0.0)
    {
        texty[currtextdraw]-=1.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_CROUCH && texty[currtextdraw] <= 480.0)
    {
        texty[currtextdraw]+=1.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_WALK+KEY_SPRINT && textx[currtextdraw] <= 640.0)
    {
        textx[currtextdraw]+=10.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_LOOK_BEHIND+KEY_SPRINT && textx[currtextdraw] >= 0.0)
    {
        textx[currtextdraw]-=10.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_JUMP+KEY_SPRINT && texty[currtextdraw] >= 0.0)
    {
        texty[currtextdraw]-=10.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_CROUCH+KEY_SPRINT && texty[currtextdraw] <= 480.0)
    {
        texty[currtextdraw]+=10.0;
        UpdateTextDraw(currtextdraw,1);
    }
    if(movingtext && keys == KEY_ACTION)
    {
        SendClientMessage(playerid,0x554466AA,"Text sucessfully placed!");
        TogglePlayerControllable(playerid,1);
        movingtext=0;
    }
    if(lettersizing && keys == KEY_WALK && textx[currtextdraw] <= 640.0)
    {
        lettersizex[currtextdraw]+=0.1;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_LOOK_BEHIND && textx[currtextdraw] >= 0.0)
    {
        lettersizex[currtextdraw]-=0.1;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_JUMP && texty[currtextdraw] >= 0.0)
    {
        lettersizey[currtextdraw]-=0.1;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_CROUCH && texty[currtextdraw] <= 480.0)
    {
        lettersizey[currtextdraw]+=0.1;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_WALK+KEY_SPRINT && textx[currtextdraw] <= 640.0)
    {
        lettersizex[currtextdraw]+=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_LOOK_BEHIND+KEY_SPRINT && textx[currtextdraw] >= 0.0)
    {
        lettersizex[currtextdraw]-=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_JUMP+KEY_SPRINT && texty[currtextdraw] >= 0.0)
    {
        lettersizey[currtextdraw]-=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_CROUCH+KEY_SPRINT && texty[currtextdraw] <= 480.0)
    {
        lettersizey[currtextdraw]+=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(lettersizing && keys == KEY_ACTION)
    {
        SendClientMessage(playerid,0x554466AA,"Text sucessfully placed!");
        TogglePlayerControllable(playerid,1);
        lettersizing=0;
    }
    if(boxsizing && keys == KEY_WALK && textx[currtextdraw] <= 640.0)
    {
        boxsizex[currtextdraw]+=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_LOOK_BEHIND && textx[currtextdraw] >= 0.0)
    {
        boxsizex[currtextdraw]-=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_JUMP && texty[currtextdraw] >= 0.0)
    {
        boxsizey[currtextdraw]-=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_CROUCH && texty[currtextdraw] <= 480.0)
    {
        boxsizey[currtextdraw]+=1.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_WALK+KEY_SPRINT && textx[currtextdraw] <= 640.0)
    {
        boxsizex[currtextdraw]+=10.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_LOOK_BEHIND+KEY_SPRINT && textx[currtextdraw] >= 0.0)
    {
        boxsizex[currtextdraw]-=10.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_JUMP+KEY_SPRINT && texty[currtextdraw] >= 0.0)
    {
        boxsizey[currtextdraw]-=10.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_CROUCH+KEY_SPRINT && texty[currtextdraw] <= 480.0)
    {
        boxsizey[currtextdraw]+=10.0;
        UpdateTextDraw(currtextdraw,0);
    }
    if(boxsizing && keys == KEY_ACTION)
    {
        SendClientMessage(playerid,0x554466AA,"Box size successfully changed, changing color");
        boxsizing=0;
        menu=5;
    ...
    if (keys > 0) SetTimerEx("HoldKey", 300, 0, "d", playerid);
}
